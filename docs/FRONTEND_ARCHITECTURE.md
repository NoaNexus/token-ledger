...
后台数据刷新保持页面亮度不变；扫描进度轮询只更新状态，不重建卡片。独立的装饰背景层承载随指针移动的镜面高光及 ±4.2° 俯仰，文字和卡片容器不做透视变换，避免文字随整卡重采样发虚。移开后背景回到平面；高光定位使用四边属性兼容旧内核，减少动画偏好下禁用旋转和镜面动画。概览纵向间距使用 Grid gap（28px），不依赖旧内核不支持的 Flex gap；费用说明和金额分行，窄窗口自动减少 KPI 列数。
